...
export default function CreateContent() {
  ...
  const [interest, setInterest] = useState(fallbackInterestOptions);
  const [currentProcess, setCurrentProcess] = useState(0);
  const hasStartedPrediction = React.useRef(false);

  useEffect(() => {
    if (location.state?.resetCreateContent) {
      setCurrentStep(0);
      setCurrentProcess(0);
      setLoading(false);
  ...
  useEffect(() => {
    api.get('/plan/interest')
      .then((data) => {
        const nextInterests = Array.isArray(data?.interests) ? data.interests : [];
        setInterest(nextInterests.length ? nextInterests : fallbackInterestOptions);
      })
      .catch((err) => {
        console.error('Error fetching interest data:', err);
        setInterest(fallbackInterestOptions);
      })
            ...
            <div className="col-span-3">
              ...
              <div className="mt-8 rounded-xl border-2 px-5 py-5 bg-white border-[#dddddd]">
                ...
                <div className="mt-5 gap-3 flex flex-wrap">
                  {interest.map((option) => {
                    const value = typeof option === 'string' ? option : option.interest_name || option.name || '';
                    const isSelected = Array.isArray(inputData.interests) && inputData.interests.includes(value);
                    ...
                    return (
                      <button
                        key={value}
                        type="button"
                        onClick={() => updateInputField('interests', value)}
                      ...
                      >
                        <div className={`text-[16px] font-medium ${isSelected ? 'text-[#ffffff]' : 'text-[#444444]'}`}>{value}</div>
                      </button>
                    );
                  })}
                </div>
  ...
  return (
    <div className="min-h-screen bg-[#fafaff] text-[#172033]">
      <div className="flex min-h-screen">
        ...
        <main className="relative min-w-0 flex-1 pt-[72px] transition-all duration-300 ease-in-out lg:ml-[180px]">
          {isSidebarOpen && (
            <button
              type="button"
              ...
              <section className="mt-[-2rem] px-6 py-8 sm:px-8 lg:px-10 lg:py-10">
                ...
                <div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:items-center">
                  ...
                  <button
                    ...
                    className="items-center justify-center rounded-lg bg-[#4f46e5] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#4338ca] disabled:cursor-not-allowed disabled:opacity-40 sm:ml-auto"
                  >
