...
export default function Navbar() {
  ...
  if (isAuthenticated) {
    return <header className="sticky top-0 z-40 border-b border-[#e7e5f1]/90 bg-[#fbfaff]/95 backdrop-blur-md"><div className="flex min-h-[72px] items-center justify-between px-4 sm:px-8"><Link to="/dashboard" className="text-xl font-extrabold tracking-[-0.04em] text-[#6d5ce7]">Meateka</Link><div className="flex items-center gap-1.5"><NotificationMenu /><SettingsButton /><div className="relative ml-1"><UserAvatar user={user} isOpen={isProfileOpen} onClick={() => setIsProfileOpen((open) => !open)} />{isProfileOpen && <ProfileDropdown user={user} onClose={() => setIsProfileOpen(false)} />}</div></div></div></header>;
  }
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#e7e5f1]/90 bg-[#fbfaff]/95 backdrop-blur-md">
      <div className="mx-auto flex min-h-[72px] max-w-7xl items-center justify-between px-5 sm:px-8">
        <Link to="/" className="text-2xl font-extrabold tracking-[-0.04em] text-[#6d5ce7]">Meateka</Link>
